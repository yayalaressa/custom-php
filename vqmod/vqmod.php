...
abstract class VQMod {
	...
	public static function modCheck($sourceFile, $modificationFile = false) {
		...
		if (sha1($fileData) != $fileHash) {
			...
			file_put_contents(self::path(self::$checkedCache, true), $stripped_filename . PHP_EOL);
	...
	private static function _getMods() {
		...
		file_put_contents(self::path(self::$checkedCache, true), '');
...
class VQModLog {
	...
	public function __destruct() {
		...
		$result = file_put_contents($logPath, implode(PHP_EOL, $txt));
		if(!$result) {
